...
impl MonitorConfiguration{
    pub fn load_from_file(path: &str) -> Option<Box<MonitorConfiguration>>{
        let filepath = Path::new(path);
        let mut read_file = File::open(filepath).ok()?;
        let mut file_contents= String::new();
        read_file.read_to_string(&mut file_contents).ok()?;
        let file_data = json::parse(file_contents.as_str()).unwrap();
        ...
        for i in 0..monitor_configurations.len(){
            ...
            let monitor_instance = Box::new(Monitor{
